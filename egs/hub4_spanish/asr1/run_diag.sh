...
mtlalpha=0.5
mtlalpha_decay=0.9999

# label smoothing
lsm_type=unigram
lsm_weight=0.05
schedule_sampling_prob=0.0
...
lm_weight=1.0
use_lm=false
lmexpdir=exp/train_rnnlm_pytorch_
...
if [ ${stage} -le 1 ]; then
    ...
    utils/create_split_dir.pl \
        /export/b{10,11,14,13}/${USER}/espnet-data/egs/hub4_spanish/asr1/dump/${train_set}/delta${do_delta}/storage \
        ${feat_tr_dir}/storage
    fi
    if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d ${feat_dt_dir}/storage ]; then
    utils/create_split_dir.pl \
        /export/b{10,11,14,13}/${USER}/espnet-data/egs/hub4_spanish/asr1/dump/${train_dev}/delta${do_delta}/storage \
        ${feat_dt_dir}/storage
    fi
...
if [ ${stage} -le 4 ]; then
    ...
    #${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        ...
        --mtlalpha ${mtlalpha} \
        --mtlalpha-decay ${mtlalpha_decay} \
        --lsm-type ${lsm_type} \
        --lsm-weight ${lsm_weight} \
        --sampling-probability ${schedule_sampling_prob} \
