...
from e2e_asr_th_diagnostic import Loss
from e2e_asr_th_diagnostic import E2E
from e2e_asr_th_diagnostic import pad_list
...
class CustomUpdater(training.StandardUpdater):
    ...
    def __init__(self, model, grad_clip_threshold, train_iter,
                 optimizer, converter, device, ngpu, output):
    ...
    def update_core(self):
        ...
        if (self.iteration + 1) % 10 == 0:
            with open(self.output + "/delta_ctc", 'w') as f:
                json.dump(optimizer.diffs, f, sort_keys=True,
                    indent=4, separators=(',', ': '))
        ...
        loss_ctc = self.model(*x, objf='ctc')
        if self.ngpu > 1:
            loss_ctc.backward(loss.new_ones(self.ngpu))
        else:
            loss_ctc.backward()
        loss_ctc.detach()
    
        grad_norm_ctc = torch.nn.utils.clip_grad_norm_(
        ...
        for p in self.model.named_parameters():
            if p[1].grad is not None:
                grads[p[0]] = p[1].grad.clone() 
        del loss_ctc
         
        optimizer.zero_grad()  # Clear the parameter gradients
        loss_att = self.model(*x, objf='att')
        print(loss_att)
        if self.ngpu > 1:
            loss_att.backward(loss.new_ones(self.ngpu))  # Backprop
        else:
            loss_att.backward()  # Backprop
        loss_att.detach()  # Truncate the graph
        ...
        for p in self.model.named_parameters():
            if p[0] in grads:
                p[1].grad = torch.tensor(grads[p[0]])
        
        if not math.isnan(grad_norm_ctc):
            optimizer.step()
...
def train(args):
    ...
    #                      model.predictor,
    #                      converter=converter,
    #                      undo_type=args.datadelta_undo_type,
    #                      device=device,
    #                      lr=1.0,
    #                      rho=0.95,
    #                      eps=args.eps,
    #                      weight_decay=0.,
    #                      valid_batches=valid,
    #                      which_batches_to_check=args.datadelta_check_batch_types.strip().split(),
    #                      diff_threshold=0.)
    ...
    updater = CustomUpdater(
            model, args.grad_clip, train_iter, optimizer, converter, device, args.ngpu, output=args.outdir)
    ...
    trainer.extend(extensions.PlotReport(['main/loss_ctc',
    ...
    if args.opt in ('adadelta' , 'datadelta'):
    ...
    report_keys = ['epoch', 'iteration', 'main/loss_ctc', 'main/loss_att',
