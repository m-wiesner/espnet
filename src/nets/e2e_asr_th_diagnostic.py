...
class Loss(torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad, objf='att'):
        ...
        :rtype: torch.Tensor
        '''
        hs, hlens = self.predictor.enc(xs_pad, ilens)
        if objf == 'att':
            loss_att, acc = self.predictor.dec(hs, hlens, ys_pad) 
            self.reporter.report(dict([('loss_att', float(loss_att)), ('acc', acc)]))
            loss = (1 - self.mtlalpha) * loss_att
            return loss
        elif objf == 'ctc':
            loss_ctc = self.predictor.ctc(hs, hlens, ys_pad)
            self.reporter.report(dict([('loss_ctc', float(loss_ctc))]))
            loss = self.mtlalpha * loss_ctc
            return loss
        

class Discriminator(torch.nn.Module):
    def __init__(self, idim, odim, window_size=20):
        ...
        # We just flatten a patch of window_size to look at
...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, etype, idim, elayers, eunits, eprojs, subsample, dropout, in_channel=1,
                tdnn_odims=None, tdnn_offsets=None, tdnn_prefinal_affine_dim=625,
